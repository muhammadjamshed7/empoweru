...
const goalIcons = {
  "Fat Loss": Zap,
  "Strength": Dumbbell,
  "Muscle Gain": Activity, // Using Activity as a placeholder for muscle gain
};
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredPlans.length === 0 && (
        <p className="text-center text-muted-foreground dark:text-slate-400 py-4">No workout plans found for the selected goal.</p>
      )}

      <Accordion type="single" collapsible className="w-full space-y-4">
        {filteredPlans.map((plan) => {
          const PlanIcon = goalIcons[plan.goal] || Activity;
          return (
            ...
            <AccordionTrigger className="p-4 hover:no-underline data-[state=open]:bg-slate-800/50 data-[state=open]:dark:bg-slate-800">
              <div className="flex items-center space-x-4 w-full">
                ...
                <div className="text-left">
                  <h3 className="font-headline text-lg text-foreground dark:text-blue-300">{plan.title}</h3>
                  <p className="text-sm text-muted-foreground dark:text-slate-400">{plan.description}</p>
                </div>
              </div>
            </AccordionTrigger>
              ...
              <ul className="space-y-3">
                {plan.exercises.map((exercise, index) => (
                  <li key={index} className="p-3 bg-muted/50 dark:bg-slate-800/70 rounded-md">
                    <p className="font-semibold text-foreground dark:text-slate-200">{exercise.name}</p>
